...
public class Scheduler {
	 ...
	 */
	private List<Septet<Double, Double, Double, Double, Double, Long, Double>> costListIoTDevice = new ArrayList<Septet<Double, Double, Double, Double, Double, Long, Double>>();
	 ...
	 * - Variable 6 : CPU core supply voltage
	 */
	private List<Septet<Double, Double, Double, Double, Double, Long, Double>> costListMECServer = new ArrayList<Septet<Double, Double, Double, Double, Double, Long, Double>>();
	 ...
	 */
	private List<Septet<Double, Double, Double, Double, Double, Long, Double>> costListCloud = new ArrayList<Septet<Double, Double, Double, Double, Double, Long, Double>>();
	 ...
	 * Constructor
	 */
	public Scheduler(Task task, double coefficientEnergy, double coefficientTime, double alpha, double beta,
			double gamma) {
		this.task = task;
	...
	private void calculateCostPolicyIoTDevice(double alpha) {
		...
		for (Pair<Long, Double> pairFrequencyVoltage : pairsFrequencyVoltage) {
			...
			this.costListIoTDevice.add(
					new Septet<Double, Double, Double, Double, Double, Long, Double>(cost, dynamicEnergy, 0.0,
							executionTime, 0.0, pairFrequencyVoltage.getValue0(), pairFrequencyVoltage.getValue1()));
		}
	...
	private void calculateCostPolicyMECServer(double beta) {
		...
		for (Pair<Long, Double> pairFrequencyVoltage : pairsFrequencyVoltage) {
			double executionTime = this.serverMEC.calculateExecutionTime(pairFrequencyVoltage.getValue0(),
					this.task.getComputationalLoad());
			...
			double executionTimeTotal = executionTime + this.time5GUp + this.time5GDown;

			double cost = (this.coefficientEnergy * dynamicEnergyTotal + this.coefficientTime * executionTimeTotal)
					* beta;
			this.costListMECServer.add(
					new Septet<Double, Double, Double, Double, Double, Long, Double>(cost, dynamicEnergy,
							(this.energy5GUp + this.energy5GDown),
							executionTime, (this.time5GUp + this.time5GDown),
	...
	private void calculateCostPolicyCloud(double gama) {
		...
		double totalStandardEnergy = standardEnergy + this.energy5GUp + this.energyFiberUp + this.energyFiberDown
				+ this.energy5GDown;
		double totoalStandardTime = standardTime + this.time5GUp + this.timeFiberUp + this.timeFiberDown
				+ this.time5GDown;

		double standardCost = (this.coefficientEnergy * totalStandardEnergy + this.coefficientTime * totoalStandardTime)
				* gama;
		this.costListCloud.add(
				new Septet<Double, Double, Double, Double, Double, Long, Double>(standardCost, standardEnergy,
						(this.energy5GUp + this.energyFiberUp + this.energyFiberDown + this.energy5GDown),
		...
		double totalTurboEnergy = turboEnergy + this.energy5GUp + this.energyFiberUp + this.energyFiberDown
				+ this.energy5GDown;
		...
		this.costListCloud.add(
				new Septet<Double, Double, Double, Double, Double, Long, Double>(turboCost, turboEnergy,
						(this.energy5GUp + this.energyFiberUp + this.energyFiberDown + this.energy5GDown),
	 ...
	 */
	public Octet<Double, Double, Double, Double, Double, Long, Double, Integer> defineAllocationPolicy(
			boolean flagIoTDevice, boolean flagMECServer) {
		// Global cost comparator
		List<Octet<Double, Double, Double, Double, Double, Long, Double, Integer>> globalCostList = new ArrayList<Octet<Double, Double, Double, Double, Double, Long, Double, Integer>>();
		...
		if (task.getDeadline() != NORMAL_TASK) {
			...
			// Task is critical - Rearrange global list comparator to have total as the
			// first value
			List<Octet<Double, Double, Double, Double, Double, Long, Double, Integer>> globalCostListCritical = new ArrayList<Octet<Double, Double, Double, Double, Double, Long, Double, Integer>>();
			for (Octet<Double, Double, Double, Double, Double, Long, Double, Integer> octet : globalCostList) {
				Octet<Double, Double, Double, Double, Double, Long, Double, Integer> aux = new Octet<Double, Double, Double, Double, Double, Long, Double, Integer>(
						(octet.getValue3() + octet.getValue4()), octet.getValue1(), octet.getValue2(),
						octet.getValue0(),
			...
			// Alter back octet elements 0 and 3 to keep the stardard of cost being value0
			// and total time value2
			Octet<Double, Double, Double, Double, Double, Long, Double, Integer> resultOctet = new Octet<Double, Double, Double, Double, Double, Long, Double, Integer>(
					globalCostListCritical.get(0).getValue3(), globalCostListCritical.get(0).getValue1(),
					globalCostListCritical.get(0).getValue2(),
					(globalCostListCritical.get(0).getValue0() - globalCostListCritical.get(0).getValue4()),
	...
	private void printOctetList(List<Octet<Double, Double, Double, Double, Double, Long, Double, Integer>> octetList) {
		...
		if (this.task.getDeadline() != -1) {
			...
			System.out.println(
					"Cost;CPU core Energy;Transfer Energy;CPU core Time;Diff Time;Transfer Time;Frequency;Voltage;Policy");
			...
			System.out.println(
					"Cost;CPU core Energy;Transfer Energy;CPU core Time;Transfer Time;Frequency;Voltage;Policy");
	...
	public void printSystemCosts() {
		...
		int i = 0;
		for (Septet<Double, Double, Double, Double, Double, Long, Double> septet : this.costListIoTDevice) {
			System.out.println("Cost " + i + ": " + df.format(septet.getValue0()) + "; CPU core energy: "
					+ df.format(septet.getValue1()) +
					"; Transfer energy: " + septet.getValue2() + "; CPU core time: " + septet.getValue3() +
					"; Transfer time: " + septet.getValue4() + "; Frequency: " + septet.getValue5() + "; Voltage: "
					+ septet.getValue6());
			i++;
		}
