...
public class Application {
	 ...
	 * - When calling this function the application will define if the new task is
	 * critical or not
	...
	public static Map<Integer, List<String>> generateDependencies(int totalTasks,
			double dependencyProbability, Random rand) {
		Map<Integer, List<String>> dependencyMap = new HashMap<>();

		for (int i = 0; i < totalTasks; i++) {
			List<String> deps = new ArrayList<>();
			if (i > 0 && rand.nextDouble() < dependencyProbability) {
				int depIndex = rand.nextInt(i);
				deps.add("Task-" + depIndex);
			}
			dependencyMap.put(i, deps);
		}
